...
@section('content')
    ...
    <div class="card shadow-sm border-0">
        <div class="card-body">
            <table id="table-permissions" class="table table-hover mb-0">
                ...
                <tbody>
                    @forelse($permissions as $permission)
                        <tr>
                            ...
                            <td><span
                                    class="badge bg-light text-dark border">{{ explode('.', $permission->name)[0] }}</span>
                            </td>
                            ...
                            <td class="text-end">
                                ...
                                @can('permission.delete')
                                    <form method="POST" action="{{ route('admin.permissions.destroy', $permission) }}"
                                        class="d-inline" onsubmit="return confirm('Hapus permission {{ $permission->name }}?')">
                                        @csrf @method('DELETE')
...
@push('scripts')
    <script>
        let table = new DataTable('#table-permissions', {
            "columnDefs": [{
                "orderable": false,
                "targets": [3, 5]
            }]
